...
My exploration with this iOS app is primarily to test the feasibility that SAMI allows the 3rd party developers to combine the data from diverse sources. In this post, I talk about the first version. In the followup post, I will discuss how to implement the second version.
...
- Enter your user name and password to authenticate.
...
- You can refresh the screen to get the latest data.
...
[Make a note of the client ID.](/sami/sami-documentation/developer-user-portals.html#how-to-find-your-application-id) This is my application ID, which I will need later.

### Prepare source files and libraries

I uses the SAMI [iOS SDK](http://developer.samsungsami.io/sami/native-SDKs/objectivec-SDK.html) to avoid implementing REST call details.
...
Now build in Xcode and run SAMIHMonitor in iOS simulator. You should be able  play with the app in the way that has been demoed.
...
SAMI supports multiple OAuth 2.0 workflows. After consulting the [workflow examples](http://developer.samsungsami.io/sami/demos-tools/oauth2-flow-examples.html) on SAMI developer documentation site, I chose to implement 'Implicit' method. The method is straightforward to implement and sufficient for a mobile app.
...
In order to get an access token, `LoginViewController` captures the callback after the authentication succeeds. It overrides `UIWebViewDelegate`'s method. In that method, it exams if the redirct url is the right one and extracts the access token if so.
...
`UserInfoViewController` is in charge of presenting user information after the user logins in. The controller uses `SamiUserApi` class in the SAMI iOS SDK library to send the request to SAMI and presents the received user information as the following code:
...
`UserInfoViewController` also enquires SAMI to get a list of the devices that the user owns and then checks if the user has any Withings device. Based on that information, "See Weight" button is enabled or disabled on the user information screen.
...
To get the historical messages, I use the method of `SamiMessagesApi` class in the SDK per following code:
...
- (void) refreshMessages {
    ...
    [api2 getLastNormalizedMessagesWithCompletionBlock:@(20) sdids:device_._id fieldPresence:nil completionHandler:^(SamiNormalizedMessagesEnvelope *output, NSError *error) {
...
To properly present weight data and corresponding dates, I need to parse the messages in the HTTP response as below:
...
By now, I can see weight numbers showing in the table. However, what is the unit of the weight data? Based on the [SAMI documentation](http://developer.samsungsami.io/sami/sami-documentation/the-manifest.html), I can get that piece of the information from the Manifest of the Withings device. The following code use the method of `SamiDeviceTypesApi` class in the SDK to get the Manifest and then the unit by parsing the Manifest:
...
Ok, I am done with a simple iOS app that presents historical weight data from the Withings device of a logged in users. Please consult the followup post for version two of the app.

## Appendix

### Find the device type of Withings device
### Find the message data structure
